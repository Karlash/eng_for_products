...
class LearningProgress(Base):
    ...
    cycle_start_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_asked_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    next_review_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
    learned_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class AnswerLog(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    word_id: Mapped[int] = mapped_column(ForeignKey("words.id", ondelete="CASCADE"), nullable=False)
    asked_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), default=utcnow, nullable=False)
...
class BotState(Base):
    ...
    pending_asked_at: Mapped[datetime.datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
