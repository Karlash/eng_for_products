...
from app.services.claude_client import suggest_new_words
...
async def run_suggestion_top_up(force: bool = False) -> int:
    """Fetch a batch of thematically-suggested words if the new-word pool is low. Returns count added."""
    ...
    async with async_session() as session:
        ...
        topics = [
            t
            for (t,) in (
                await session.execute(select(Word.topic).distinct().where(Word.topic.is_not(None)))
            ).all()
            if t
        ]
        sample_pairs = (
        ...
        candidates = await suggest_new_words(topics, sample_pairs, settings.suggestion_batch_size)
        ...
        added = 0
        for english, russian in new_pairs:
            candidate = by_key.get(english.strip().lower())
            word = Word(
                english=english,
                russian=russian,
                part_of_speech=candidate.part_of_speech if candidate else None,
